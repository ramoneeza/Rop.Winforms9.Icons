...
namespace Rop.Winforms9.DuotoneIcons
{
    internal static class Converter
    {
        ...
        public static void From32BTo4B(ReadOnlySpan<int> bmp, Span<byte> dst)
        {
            ...
            while (i < bmp.Length)
            {
                var g0 = GetGray2B((uint)bmp[i]);
                i++;
                var g1 = GetGray2B((uint)bmp[i]);
                i++;
        ...
        public static void From4BTo32B(ReadOnlySpan<byte> bmp, Span<int> dst, int[] colors)
        {
            var i = 0;
            var f = 0;
            while (i < bmp.Length)
        ...
        public static Bitmap ToBmp4(ReadOnlySpan<byte> data, Size size, Color? color1 = null, Color? color2 = null)
        {
            ...
            bmp.Palette = pal;
            var bmpData = bmp.LockBits(new Rectangle(0, 0, size.Width, size.Height), ImageLockMode.WriteOnly,
